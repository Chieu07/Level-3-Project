    ...
    if username in r.keys() and password == r[username]: # If the password match with the file then it will create the main app window
        ...
        def deleteTask():
            ...
            if task in task_list:
                ...
                with open("tasklist.txt", 'w') as taskfile:  #The task will be save in other file
...
def signup_command():
    ...
    def on_leave(e):
        if user_name.get() == '':
            user_name.insert(0,"First Name")  #If the user focus out the entry box then the username word will rewrite 
